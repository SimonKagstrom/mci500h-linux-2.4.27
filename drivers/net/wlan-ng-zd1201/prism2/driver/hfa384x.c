...
static int hfa384x_zd1201_reset(hfa384x_t *hw);

/*================================================================*/
/* Local Static Definitions */
extern int prism2_debug;
#ifdef ZD1201_CHIPSET
static atomic_t zd1201_in_reset = ATOMIC_INIT(1);
struct timer_list zd1201_reset_timer;
DECLARE_TASKLET(zd1201_reset_tasklet, 
		(void (*)(unsigned long))hfa384x_zd1201_reset, 0);
#endif
...
#ifdef ZD1201_CHIPSET /* Function only used for ZD1201 modifications */
/*----------------------------------------------------------------
* hfa384x_sleep
*
* Sleeps for a specified number of centiseconds by scheduling a
* timeout.  This function is useful for large timeouts.  The
* argument is in centiseconds since that is the default resolution
* of Linux scheduler on most platforms.
*
* Arguments:
*   n_cs      number of centiseconds
*
* Returns: 
*	nothing
*
 ...
 */
static void hfa384x_sleep (unsigned int n_cs)
{
...
{
	int		result = 0;
	hfa384x_saved_rec_t *rec = NULL;
	struct list_head *pos;
	DBFENTER;

	result = hfa384x_cmd_access( hw, 1, rid, buf, len);
	if (result == 0) {
		/* Check if this already has an entry */
		list_for_each(pos, &hw->record_list) {
		...
		if (!result) {
			...
			rec->rid = rid;
			rec->buf = kmalloc(len, GFP_KERNEL);
			list_add_tail(&rec->list, &hw->record_list);
		} else {
			if (rec->buf != NULL && rec->len != len) {
				kfree(rec->buf);
				rec->buf = kmalloc(len, GFP_KERNEL);
			} 
		}
		if (rec->buf == NULL) {
			/* Allocation failed for this entry */
			WLAN_LOG_ERROR("Could not allocate memory for storing RID %#x\n", rid);
			list_del(&rec->list);
			kfree(rec);
			goto setc_nomem;
		}
...
int hfa384x_drvr_ramdl_enable(hfa384x_t *hw, UINT32 exeaddr)
{
	...
	if (exeaddr == 0x3f0000) {
		...
		UINT16 readbuf[2];
		UINT8 hcr = 0x1f; // default to 2x8 sram
		...
		if (memcmp(initseq, readbuf, sizeof(readbuf))) {
			hcr = 0x0f;
			hw->isram16 = 1;
			WLAN_LOG_DEBUG(1, "Card has1x16 SRAM\n"); 
...
int hfa384x_drvr_ramdl_write(hfa384x_t *hw, UINT32 daddr, void* buf, UINT32 len)
{
	...
	WLAN_LOG_DEBUG(1, "Writing %ld bytes to ram @0x%06lx\n", len, daddr);
#if 0
...
#ifdef ZD1201_CHIPSET
/*----------------------------------------------------------------
* hfa384x_drvr_zd1201_enable
*
* Disables the AUX port and executes a resume on the Zydas
* 1201 chip.  It then tries to initialize the CPU.  If the
* initialize is successful, the firmware is tagged as active.
*
*
*
* Arguments:
...
*	process thread 
----------------------------------------------------------------*/
int hfa384x_drvr_zd1201_enable(hfa384x_t *hw)
{
	int		result = 0;
	DBFENTER;

	/* Disable the aux port */
	if ( (result = hfa384x_cmd_aux_disable(hw)) ) {
		goto failed;
	}

	/* Unhalt the CPU */
	hfa384x_setreg(hw, ZD1201_RESUME_CPU, HFA384x_CMD);
	hw->dlstate = HFA384x_DLSTATE_ZD1201;

	/* Sleep for 5 centiseconds (50 ms) to allow the CPU to fully wakeup */
	if (!in_interrupt()) {
		hfa384x_sleep(ZD1201_WAKEUP_TIME);
	} 
	hw->zd1201_fwactive = 1;
    
	/* If we are coming out of DLSTATE_ZD1201 reset dlstate */
	if ( hw->dlstate == HFA384x_DLSTATE_ZD1201 ) {
        hw->dlstate = HFA384x_DLSTATE_DISABLED;
	}
...
/*----------------------------------------------------------------
* hfa384x_drvr_zd1201_disable
*
* Halts the CPU on the Zydas 1201 chip and enables the AUX register.
* This function must be executed before attempting to download the
* primary firmware onto the chip.
*
* Arguments:
*	hw		device structure
*
...
*	process thread 
----------------------------------------------------------------*/
int hfa384x_drvr_zd1201_disable(hfa384x_t *hw)
{
	int		result = 0;
	int		i;
	...
	/* Halt the CPU */
	hfa384x_setreg(hw, ZD1201_HALT_CPU, HFA384x_CMD);

	/* Wait a short amount of time for safety */
	udelay(10);
...
/*----------------------------------------------------------------
* hfa384x_drvr_zd1201_write
*
* Performs a download of a chunk of firmware. First checks to see
* that we're in the appropriate download state, then sets the download
* mode, uses the aux functions to 1) copy the data to the chip's
* RAM, 2) readback, and 3) compare.
*
* Repeat as many times an necessary to get all the firmware into RAM.
* When all data has been written using this function (possibly 
* repeatedly), call drvr_zd1201_disable() to end the download state
* and re-initialize the Zydas 1201.
*
* Arguments:
*	hw		device structure
*	daddr		Card address to write to. (host order)
...
*	process thread 
----------------------------------------------------------------*/
int hfa384x_drvr_zd1201_write(hfa384x_t *hw, UINT32 daddr, void* buf, UINT32 len)
{
	int		result = 0;
	int		ofs = 0, clen;
	UINT8		*verbuf;
...
#endif
	/* Copy the data via the aux port */
	hfa384x_copy_to_aux(hw, daddr, 0, buf, len);

	/* Write the plain text watermark */
	hfa384x_copy_to_aux(hw, ZD1201_WATERMARK_PLAIN_OFFSET, 0, 
				(void *)zd1201_watermark_plaintext,
				sizeof(zd1201_watermark_plaintext));

	/* Write the cipher text watermark */
	hfa384x_copy_to_aux(hw, ZD1201_WATERMARK_CIPHER_OFFSET, 0, 
				(void *)zd1201_watermark_ciphertext,
				sizeof(zd1201_watermark_ciphertext));

	/* Create a buffer for the verify */
	verbuf = kmalloc(PAGE_SIZE, GFP_ATOMIC);
	if (verbuf == NULL ) {
		WLAN_LOG_ERROR("Could not allocate verify buffer.\n");
		return -ENOMEM;
	}

	while (ofs < len) {
		clen = len - ofs < PAGE_SIZE ? len - ofs : PAGE_SIZE;

		/* Read back and compare */
		hfa384x_copy_from_aux(hw, daddr + ofs, 0, verbuf, clen);
		if ( memcmp(buf + ofs, verbuf, clen) ) {
			WLAN_LOG_ERROR("firmware download verify failed!\n");
			result = -EINVAL;
			goto fwdld_failed;
		}
		ofs += clen;
	...
	/* Verify the plain watermark */
	hfa384x_copy_from_aux(hw, ZD1201_WATERMARK_PLAIN_OFFSET, 0, verbuf, 
			sizeof(zd1201_watermark_plaintext));
	if (memcmp(zd1201_watermark_plaintext, verbuf, 
				sizeof(zd1201_watermark_plaintext))) {
		WLAN_LOG_ERROR("firmware plain text watermark download "
				"verify failed!\n");
		result = -EINVAL;
		goto fwdld_failed;
	}

	/* Verify the cipher watermark */
	hfa384x_copy_from_aux(hw, ZD1201_WATERMARK_CIPHER_OFFSET, 0, verbuf, 
			sizeof(zd1201_watermark_ciphertext));
	if (memcmp(zd1201_watermark_ciphertext, verbuf, 
				sizeof(zd1201_watermark_ciphertext))) {
		WLAN_LOG_ERROR("firmware cipher text watermark download "
				"verify failed!\n");
		result = -EINVAL;
		goto fwdld_failed;
	}
...
#else
    /*
     * The reclaim feature doesn't seem to work on the ZD1201
     * so we don't use it.  The fid is manually reclaimed in
     * the int_tx or int_txexc functions.
     */
	cmd.cmd = HFA384x_CMD_CMDCODE_SET(HFA384x_CMDCODE_TX);
#endif
	cmd.parm0 = fid;
...
#ifdef ZD1201_CHIPSET
    /*
     * Not sure if this is necessary but it is prescribed in
     * the programmer's guide.
     */
	hfa384x_setreg(hw, fid, HFA384x_TXCOMPLFID);
#endif
...
int hfa384x_cmd_aux_enable(hfa384x_t *hw)
{
	...
	/*
     * For the ZD1201 chipset, don't check the command
     * register to see if it is not busy.  This can be safely
     * skipped.  In fact there is a subtle bug below.  In the
     * case that the CMD busy bit is never cleared by the
     * hardware, the timeout based on 'jiffies' can never
     * expire since interrupts have been turned off by
     * spin_lock_irqsave.
     */
#ifndef ZD1201_CHIPSET
	/* acquire the lock */
...
#else
	hfa384x_setreg(hw, HFA384x_AUXPW0, HFA384x_PARAM0);
	hfa384x_setreg(hw, HFA384x_AUXPW1, HFA384x_PARAM1);
	hfa384x_setreg(hw, HFA384x_AUXPW2, HFA384x_PARAM2);
#endif
...
*	interrupt
----------------------------------------------------------------*/
int hfa384x_copy_from_bap (hfa384x_t *hw, UINT16 bap, UINT16 id, UINT16 offset, void *buf, UINT len)
{
	int		result = -EINVAL;
	...
	{
		if (((unsigned long) buf) & 0x01)
			WLAN_LOG_DEBUG (1, "hfa384x_copy_from_bap: dst "
						"buffer is not aligned\n");

		offset_current = offset;
		offset_end = offset_current + len;

		selectreg = (bap == 0) ?  HFA384x_SELECT0 : HFA384x_SELECT1;
		offsetreg = (bap == 0) ?  HFA384x_OFFSET0 : HFA384x_OFFSET1;
		datareg   = (bap == 0) ?  HFA384x_DATA0   : HFA384x_DATA1;

		disable_irq(hw->irq);
read_retry:
		hfa384x_setreg (hw, id, selectreg);
		hfa384x_setreg (hw, offset_current, offsetreg);
		...
		while (1) {
			...
			if (HFA384x_OFFSET_ISERR(reg)) {
				WLAN_LOG_ERROR("Error setting offset in read_from_bap()");
				goto read_failure;
			}
			...
			if (++i >= 15000) {
				WLAN_LOG_ERROR("Timeout setting read buffer id = %d, offset = 0x%04x, i = %d\n", bap, offset, i);
				zd1201_reset_tasklet.data = (unsigned long)hw;
				tasklet_schedule(&zd1201_reset_tasklet);
				goto read_failure;
			}
			udelay (1);
		}

#if defined (CONFIG_SSA_SIIG) || defined (CONFIG_SSA_WESSLI)

		while (offset_current < offset_end)
		{
			reg = hfa384x_getreg_wait_detect (hw, datareg);
			
			if (reg > 0xFFFF)		/* read access failed. Go back and retry again from current offset */
				goto read_retry;
			
			if ((offset_end - offset_current) >= 2) {
				*((UINT16*) d) = reg;
				d += 2;
			}
			else {
		...
		result = has7752_pccard_io_read_multi (datareg, len, (unsigned short *) buf);
...
read_failure:		
	...
	if (result != 0) {
		WLAN_LOG_DEBUG (1, "copy_from_bap(0x%04x, 0, %d) failed, result=0x%x\n", 
				  reg, len, (unsigned int) result);
	}
...
*	interrupt
----------------------------------------------------------------*/
int hfa384x_copy_to_bap (hfa384x_t *hw, UINT16 bap, UINT16 id, UINT16 offset, void *buf, UINT len)
{
	return hfa384x_copy_to_bap4 (hw, bap, id, offset, buf, len, NULL, 0, NULL, 0, NULL, 0);
}

#if 0
static unsigned int  bap_bounce_align;
static unsigned char bap_bounce[HFA384x_BAP_DATALEN_MAX];
#endif
...
//	printk(KERN_DEBUG "ctb1 %d id %04x o %d %d %d %d %d\n", bap, id, offset, len1, len2, len3, len4);
	    ...
	    (len1+len2+len3+len4 <= HFA384x_BAP_DATALEN_MAX))
	{
...
#if 0
		if ((buf != NULL) && (len1 > 0) && ((((unsigned long) buf) | len1) & 0x01))
			printk ("hfa384x_copy_to_bap4() buf = 0x%0x8, len1 = %d\n", (unsigned long) buf, len1);

		if ((buf2 != NULL) && (len2 > 0) && ((((unsigned long) buf2) | len2) & 0x01))
			printk ("hfa384x_copy_to_bap4() buf2 = 0x%0x8, len2 = %d\n", (unsigned long) buf2, len2);

		if ((buf3 != NULL) && (len3 > 0) && ((((unsigned long) buf3) | len3) & 0x01))
			printk ("hfa384x_copy_to_bap4() buf3 = 0x%0x8, len3 = %d\n", (unsigned long) buf3, len3);

		if ((buf4 != NULL) && (len4 > 0) && ((((unsigned long) buf4) | len4) & 0x01))
			printk ("hfa384x_copy_to_bap4() buf4 = 0x%0x8, len4 = %d\n", (unsigned long) buf4, len4);
#endif
		...
		offset_current = offset;
		offset_end = offset_current + len1 + len2 + len3 + len4;
		bufnum = -1;
		curlen = len1;
		curidx = 0;
		ptr = buf;
...
write_retry:
		...
		while (1) {
			...
			if (++i >= 15000) {
				WLAN_LOG_ERROR("Timeout setting write buffer id = %d, offset = 0x%04x, i = %d\n", bap, offset, i);
				zd1201_reset_tasklet.data = (unsigned long)hw;
				tasklet_schedule(&zd1201_reset_tasklet);
				goto write_failure;
			}
...
//		printk ("offset setup complete\n");

#ifdef CONFIG_SSA_HAS7752
		result = 0;
		while (bufnum < 3 && !result) {
			result = has7752_pccard_io_write_multi (datareg, curlen, (unsigned short *) ptr);
			if (result)
				result = -EIO;
			if (++bufnum < 3) {
				curlen = buflens[bufnum];
				ptr = bufs[bufnum];
			}
		}
#else
		while (offset_current < offset_end)
		{
			...
			if (((unsigned long)ptr & 1) || ((curlen - curidx) < 2)) {
				b[0] = ptr[0];
				b[1] = ((curlen - curidx) >= 2) ? ptr[1] : 0;
			} else {
				data = *((UINT16 *)ptr);
			}
			
			if (hfa384x_setreg_wait_detect (hw, data, datareg) > 0xFFFF)
				goto write_retry;

			/* write succeeded - _now_ its safe to update the dst offset and source pointer */
			offset_current += 2;
			curidx += 2;
			ptr += 2;
			if (curlen - curidx <= 0) {
...
#if defined(ZD1201_CHIPSET) && defined(CONFIG_ARCH_SSA)
static int zd1201_pcmcia_attribute_write (hfa384x_t *hw,
                                          unsigned int addr, 
                                          unsigned char value)
{
#if defined (CONFIG_SSA_HAS7752)

	return has7752_pccard_attribute_write (addr, value);

#elif defined (CONFIG_SSA_SIIG) || defined (CONFIG_SSA_WESSLI)

	int i;
	int result;

   	*((volatile unsigned int *)(IO_ADDRESS_SMC_BASE + 8)) = ZD1201_SMC_WAITSTATES_PCMCIA_SLOW;

	gpio_set_low ((unsigned int)(GPIO_PCMCIA_MEM_SELECT | 
				     GPIO_PCMCIA_SPEED | 
				     GPIO_PCMCIA_8BIT_SELECT));

	result = -ETIMEDOUT;
	for (i = 0; i < 3; i++) {
		if (hfa384x_setreg_wait_detect (hw, value, addr) <= 0xffff) {
			result = 0;
			break;
		}
	}                
	...
	if (result != 0)
		WLAN_LOG_ERROR("Could not set PCMCIA I/O mode.\n");

	gpio_set_high ((unsigned int)(GPIO_PCMCIA_MEM_SELECT |
				      GPIO_PCMCIA_SPEED |
				      GPIO_PCMCIA_8BIT_SELECT));

	*((volatile unsigned int *)(IO_ADDRESS_SMC_BASE + 8)) = ZD1201_SMC_WAITSTATES_PCMCIA_FAST;
...
static int zd1201_pcmcia_reset (hfa384x_t *hw)
{
	int i;
	unsigned long flags;
	...
	/* Drive high to assert PCMCIA reset */
	gpio_set_as_output (GPIO_PCMCIA_RESET, 1); 

	/* Hold it high for 10us */
	udelay (10);

	/* drive low to deassert PCMCIA reset */
	gpio_set_as_output (GPIO_PCMCIA_RESET, 0); 

#if defined (CONFIG_SSA_SIIG) || defined (CONFIG_SSA_WESSLI)
	gpio_int_set_type (GPIO_PCMCIA_WAIT, GPIO_FALLING_EDGE_TRIGGERED);
#endif

	/* Reset should be completed within 100ms */
	for (i = 1000/10 * 100; i > 0; i++) {
		/* high value on IRQ pin means the card has finished its 
		 * reset sequence.
		 */
		if (gpio_get_value (GPIO_PCMCIA_INTRQ) == 1)
			break;
		udelay(10);
	}

	if (i == 0) {
		WLAN_LOG_ERROR("Timeout waiting for zd1201 to de-assert HREADY\n");
		local_irq_restore(flags);
		return -1;
	}

	/* Finally, set the card in I/O mode. 0x1FE is the address of the 
	 * config register (taken from CISTPL_CONFIG) 
	 */
	i = zd1201_pcmcia_attribute_write(hw, 0x1FE, 0x01);

	local_irq_restore(flags);
	return i;
}

static int hfa384x_zd1201_reset_helper(hfa384x_t *hw)
{
	unsigned char *fw = hw->zd1201_fwbuf;
	unsigned int length = ((firmware_header_t *) fw)->image_length;
	int i = 0;

retry_reset:
	hw->dlstate = HFA384x_DLSTATE_DISABLED;
	if (hfa384x_drvr_zd1201_disable(hw)) {
		WLAN_LOG_ERROR("Unable to disable interface.\n");
		return -1;
	}

	/* Make 'fw' point to the start of firmware _after_ the header */
	fw += sizeof(firmware_header_t); 
	if (hfa384x_drvr_zd1201_write(hw, ZD1201_FIRMWARE_OFFSET, 
							fw, length)) {
		if (i++ == 5) {
			WLAN_LOG_ERROR("Firmware download failed.\n");
			hfa384x_cmd_aux_disable(hw);
			return -1;
		}
		goto retry_reset;
	...
	if (hfa384x_drvr_zd1201_enable(hw)) {
		WLAN_LOG_ERROR("Unable to enable interface\n");
		return -1;
	}
	return 0;
}

static int hfa384x_zd1201_reset_prologue(unsigned long data)
{
	struct list_head *pos;
	...
	/* Start the interface */
	if (hfa384x_drvr_start(hw)) {
		enable_irq(hw->irq);
		atomic_inc(&zd1201_in_reset);
		return -1;
	}
	...
	list_for_each(pos, &hw->record_list) {
		rec = list_entry(pos, hfa384x_saved_rec_t, list);
		if (hfa384x_cmd_access(hw, 1, rec->rid, rec->buf, rec->len)) {
			WLAN_LOG_ERROR("Failed to restore config. RID = %#x\n",
					rec->rid);
			enable_irq(hw->irq);
			atomic_inc(&zd1201_in_reset);
			return -1;
...
static int hfa384x_zd1201_reset(hfa384x_t *hw)
{
	...
	if (!atomic_dec_and_test(&zd1201_in_reset)) {
		WLAN_LOG_DEBUG(1, "Already in reset function, exiting\n");
		atomic_inc(&zd1201_in_reset);
		return 0;
	}
	/* Stop the Transmit queue */
	netif_stop_queue(hw->wlandev->netdev);
	...
	if (in_interrupt()) {
		/* The embedded ARM on the card needs 50ms to startup, so we 
		 * schedule a timeout. This is only needed in interrupt context
		 * (softirq, tasklet, bottom half and hardirq).
		 */
		init_timer(&zd1201_reset_timer);
		zd1201_reset_timer.expires = jiffies + (5 * HZ / 100);
		zd1201_reset_timer.data = (unsigned long)hw;
		zd1201_reset_timer.function = 
			(void (*)(unsigned long))hfa384x_zd1201_reset_prologue;
		add_timer(&zd1201_reset_timer);
	} else {
		/* We just call the last part of the reset sequence */
		result = hfa384x_zd1201_reset_prologue((unsigned long)hw);
	}
	return result;
}
#endif
...
//			printk ("docmd evstat = 0x%04x\n", reg);
 ...
 failed:
	...
	if (result == -ETIMEDOUT) {
		/* Schedule the reset via a tasklet */
		zd1201_reset_tasklet.data = (unsigned long)hw;
		tasklet_schedule(&zd1201_reset_tasklet);
	}
	DBFEXIT;
...
#define HCR_OFFSET	0x3e2	/* HCR attribute offset of Prism2 PC card */
	...
	DBFENTER;
        ...
        WLAN_LOG_DEBUG (1, "HFA384x_INTEN  : 0x%04x\n", hfa384x_getreg (hw, HFA384x_INTEN));
        WLAN_LOG_DEBUG (1, "HFA384x_EVSTAT : 0x%04x\n", hfa384x_getreg (hw, HFA384x_EVSTAT));

        hfa384x_setreg (hw, 0x0000, HFA384x_INTEN);
...
int hfa384x_drvr_stop(hfa384x_t *hw)
{
	...
	del_timer_sync(&zd1201_reset_timer);
	tasklet_kill(&zd1201_reset_tasklet);
...
#define DOBOTH 1
	...
	if (result)
        WLAN_LOG_DEBUG(2, "txfid_queue_empty!\n");
failed:
...
void hfa384x_int_infdrop(wlandevice_t *wlandev)
{
	...
	/*
     * We are manually reclaiming this fid because the reclaim bit
     * was not used when issuing the notify command.  There seems
     * to be something wrong with the reclaim feature.
     */
	if ( reg == hw->infofid ) {
		up(&hw->infofid_sem);
    }
...
void hfa384x_int_txexc(wlandevice_t *wlandev)
{
	...
	/*
     * We are manually reclaiming this fid because the reclaim bit
     * was not used when issuing the transmit command.  There seems
     * to be something wrong with the reclaim feature.
     */
	result = txfid_queue_add(hw, fid);
    /* No need to check the result since there must be space in the queue */
...
#ifdef ZD1201_CHIPSET
	if (hfa384x_zd1201_reset(hw))
		WLAN_LOG_ERROR("Error resetting hardware\n");
#else
