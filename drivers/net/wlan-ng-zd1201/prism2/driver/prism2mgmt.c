...
int prism2mgmt_scan_results(wlandevice_t *wlandev, void *msgp)
{
	...
	if (! hw->scanresults) {
		WLAN_LOG_ERROR("dot11req_scan_results can only be used adter a successful dot11req_scan.\n");
		result = 2;
...
/*----------------------------------------------------------------
* prism2mgmt_zd1201_state
*
* Establishes the beginning/end of an intial Zydas 1201 firmware
* download.
*
* It is expected that the zd1201_write() function will be called 
* one or more times between the 'enable' and 'disable' calls to
* this function.
*
* Arguments:
...
*	process thread
----------------------------------------------------------------*/
int prism2mgmt_zd1201_state(wlandevice_t *wlandev, void *msgp)
{
	hfa384x_t		*hw = wlandev->priv;
	p80211msg_p2req_zd1201_state_t	*msg = msgp;
	DBFENTER;

	if (wlandev->msdstate != WLAN_MSD_FWLOAD) {
		WLAN_LOG_ERROR(
			"zd1201_state(): may only be called "
			"in the fwload state.\n");
		msg->resultcode.data = 
			P80211ENUM_resultcode_implementation_failure;
		msg->resultcode.status = P80211ENUM_msgitem_status_data_ok;
	...
	*/
	...
	if  ( msg->enable.data == P80211ENUM_truth_true ) {
		...
		hfa384x_drvr_zd1201_disable(hw);
		msg->resultcode.data = P80211ENUM_resultcode_success;
	}
...
/*----------------------------------------------------------------
* prism2mgmt_zd1201_fwwrite
*
* Writes a buffer to the card firmware.  This is for writing the
* initial Zydas 1201 firmware to the cardcode to card RAM.  It will
* perform a read back from the aux registers to verify the write.
*
* Arguments:
*	wlandev	 wlan device structure
*	msgp	 ptr to msg buffer
...
*	process thread
----------------------------------------------------------------*/
int prism2mgmt_zd1201_fwwrite(wlandevice_t *wlandev, void *msgp)
{
	hfa384x_t		*hw = wlandev->priv;
	p80211msg_p2req_ramdl_write_t	*msg = msgp;
	UINT32			addr;
	...
	if (wlandev->msdstate != WLAN_MSD_FWLOAD) {
		WLAN_LOG_ERROR(
			"zd1201_fwwrite(): may only be called "
			"in the fwload state.\n");
		msg->resultcode.data = 
			P80211ENUM_resultcode_implementation_failure;
		msg->resultcode.status = P80211ENUM_msgitem_status_data_ok;
	...
	len = msg->len.data;
	buf = msg->data.data;
	if ( hfa384x_drvr_zd1201_write(hw, addr, buf, len) ) {
		msg->resultcode.data = P80211ENUM_resultcode_refused;
...
int prism2mgmt_wlansniff(wlandevice_t *wlandev, void *msgp)
{
	...
	case P80211ENUM_truth_true:
		...
		if (msg->channel.status == P80211ENUM_msgitem_status_data_ok) {
