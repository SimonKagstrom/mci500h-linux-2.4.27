...
#define		HFA384x_DLSTATE_ZD1201			6
#endif

/*--- Register I/O offsets --------------------------*/
#if ((WLAN_HOSTIF == WLAN_PCMCIA) || (WLAN_HOSTIF == WLAN_PLX) || (WLAN_HOSTIF == WLAN_MMIO))
...
#ifdef ZD1201_CHIPSET
    /* Flag that specifies whether the firmware has come up or not */
	int 			zd1201_fwactive;
	void 			*zd1201_fwbuf;
...
#ifdef ZD1201_CHIPSET
typedef struct {
	UINT16 pad1 			__WLAN_ATTRIB_PACK__;
	UINT16 pad2			__WLAN_ATTRIB_PACK__;
	UINT16 image_length		__WLAN_ATTRIB_PACK__;
} __WLAN_ATTRIB_PACK__ firmware_header_t;
#endif

typedef struct {
	UINT16 				rid;	/* Record ID */
...
#ifdef ZD1201_CHIPSET
int
hfa384x_drvr_zd1201_enable(hfa384x_t *hw);
int
hfa384x_drvr_zd1201_disable(hfa384x_t *hw);
int
hfa384x_drvr_zd1201_write(hfa384x_t *hw, UINT32 daddr, void* buf, UINT32 len);
#endif                                             
int
...
#elif ((WLAN_HOSTIF == WLAN_PCI) || (WLAN_HOSTIF == WLAN_MMIO))
#ifdef CONFIG_SSA_HAS7752
	return __le16_to_cpu (has7752_pccard_io_read (reg));
#else
...
#elif ((WLAN_HOSTIF == WLAN_PCI) || (WLAN_HOSTIF == WLAN_MMIO))
#ifdef CONFIG_SSA_HAS7752
	has7752_pccard_io_write (reg, __cpu_to_le16 (val));
#else
...
#elif ((WLAN_HOSTIF == WLAN_PCI) || (WLAN_HOSTIF == WLAN_MMIO))
#ifdef CONFIG_SSA_HAS7752
	return has7752_pccard_io_read (reg);
#else
...
#elif ((WLAN_HOSTIF == WLAN_PCI) || (WLAN_HOSTIF == WLAN_MMIO))
#ifdef CONFIG_SSA_HAS7752
	has7752_pccard_io_write (reg, val);
#else
...
/*
   16bit read and write register with WAIT violation detection.
   Note: these are not safe for big endian systems...
*/

/*----------------------------------------------------------------
* hfa384x_getreg_wait_detect
*
* Retrieve the value of one of the MAC registers.  Done here
* because different PRISM2 MAC parts use different buses and such.
*
...
hfa384x_getreg_wait_detect (hfa384x_t *hw, UINT reg)
{
	UINT32 result;
	
	result = readw (hw->membase + reg);
	if (gpio_int_get_raw_status() & GPIO_PCMCIA_WAIT)
	{
		gpio_int_clear_latch (GPIO_PCMCIA_WAIT);
		result = 0xFF000000;
	}
...
----------------------------------------------------------------*/
static inline UINT32
hfa384x_setreg_wait_detect (hfa384x_t *hw, UINT16 val, UINT reg)
{
	int i;
	...
	for (i = 3; i; i--)
	{
		if (gpio_int_get_raw_status() & GPIO_PCMCIA_WAIT)
		{
			int unstick_attempts = 64;			/* 1 should always be enough... */

			while (hfa384x_getreg_wait_detect (hw, reg) > 0xFFFF) {
				if (--unstick_attempts == 0) {
					printk ("stuck in hfa384x_setreg_wait_detect... (should never happen)\n");
					break;
				}
			}
			
			gpio_int_clear_latch (GPIO_PCMCIA_WAIT);
			return 0xFF000000;				/* any result greater than 0xFFFF indicates a wait failure */
		}
	}
	
	return 0;							/* checked 3 times and no wait detected : return success */
}
