	   ...
	   unsigned long arg, int con, struct fb_info *info)
{
	...
	if (FB_BYD_CONTRAST == cmd){
		// Set contrast level
		byd_set_contrast(arg);
		
		return 0;
	}


	if (FB_BYD_BACKLIGHT == cmd) {
		//Turn the backlight ON or OFF
		byd_set_backlight(arg);
		return 0;
	}
	...
	if (FB_RUBY_FLUSH == cmd) {
		...
		copy_from_user((void *) &pos, (void *) arg, sizeof (struct screenlocation));
		//printk("FB_RUBY_FLUSH POS: x1=%d, y1=%d, x2=%d, y2=%d\n",pos.xstart,pos.ystart,pos.xend,pos.yend);
