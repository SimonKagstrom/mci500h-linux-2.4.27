...
*
*******************************************************************************/
#ifndef _PNX0106_MP3_H
#define _PNX0106_MP3_H
 ...
 * 			
 */
#define E7B_MP3_MAX_FRAMESIZE 	(E7B_MP3_MAX_BITS_REQUIRED / 8) /* 1440 bytes */
...
typedef struct	
{
	char 			data[E7B_MP3_MAX_FRAMESIZE * E7B_MP3_NBUF]; /* one circular buffer to maximize mem usage */
} e7b_mp3_input;

typedef struct 
{
	e7b_mp3_header 	hdr;
	signed short	data[E7B_MP3_MAX_SAMPLES];
...
/* The ARM needs to poll the Epics to know when decoding is finished.
 * This setting determines the amount of time that the decoder thread sleeps when a grannule is not yet available
 * Default is one tick (10 ms) since it is fast enough for playback and it drastically lowers ARM cpu usage (>20%) 
 * It can be changed by an insmod parameter and an ioctl call to increase decode time (ex. when decoding to file) */
